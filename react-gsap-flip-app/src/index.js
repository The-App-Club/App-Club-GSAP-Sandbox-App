...
const filtersInitArray = filterData.map((e) => {
  return e.value;
});
...
const App = ({children}) => {
  ...
  useEffect(() => {
    ...
    Flip.from(state, {
      ...
      onLeave: (elements) => {
        return gsap.to(elements, {opacity: 0, scale: 0, duration: 1});
      },
    });
  }, [filter]);
  ...
  const renderFilters = () => {
    return filterData.map((e) => {
      return (
        <div className="form-check form-check-inline" key={e.key}>
  ...
  return (
    <div className="container">
      <div className="row">
        ...
        <div className="element-container">{renderElements()}</div>
      </div>
    </div>
  );
};
